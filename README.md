...
* 인게임 내에서 약발의 수치가 중요한만큼 약발의 상향이 평점 변화에 미치는 영향이 클 것이라 생각했다.
* 아래는 모먼트 클래스에서 약발이 상향된 두 선수의 예시이며, 유저들이 평가한 점수가 향상된 것을 알 수 있다. (L은 왼발 수치, R은 오른발 수치를 의미함, 수치는 5점 만점임 / 위에 있는 클래스가 모먼트, 아래에 있는 클래스가 아이콘임)
